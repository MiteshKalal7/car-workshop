...
class Dashboard extends Component {
  ...
  searchCar = (filterType = this.state.filterType, removeRecord = false) => {
    ...
    if (filterType !== '') {
      bodyData.type = filterType;
    }

    if (this.state.baNo !== '') {
      bodyData.ba_no = this.state.baNo;
    }
    if (this.state.unit !== '') {
      bodyData.unit_id = this.state.unit;
    }

    bodyData = JSON.stringify(bodyData);

    console.log(`${API_URL}searchCar`, bodyData);
    // alert(bodyData);

    fetch(`${API_URL}searchCar`, {
      method: 'POST',
      headers: {
  ...
  onRefresh = () => {
    this.setState(
      ...
      () => {
        this.searchCar(this.state.filterType, true);
      },
    );
  };
    ...
    if (param !== undefined) {
      if (param.screen !== undefined) {
        ...
        this.setState(
          ...
          () => {
            this.searchCar(param.type, true);
          },
        );
